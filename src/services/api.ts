...
export const generateArticle = async (formData: any, userId: string) => {
  try {
    ...
    const response = await fetch(`${process.env.GENERATE_ARTICLE_API_URL}`, {
      method: 'POST',
      headers: {
...
export const validateFormData = async (formData: any, userId: string): Promise<ValidationResponse> => {
  ...
  const response = await fetch(`${process.env.VALIDATE_FORM_DATA_API_URL}`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
...
export const fetchArticles = async (userId: string): Promise<ArticlesResponse> => {
  try {
    const response = await fetch(`${process.env.FETCH_ARTICLES_API_URL}?userId=${userId}`);
